...
// chai.use(sinonChai); // for should and expect, useless for assert - use sinon.assert
chai.use(chaiAsPromised); // should come last

const syncTask = () => 'syncTask result';
const asyncTask = () =>
  new Promise<string>((resolve) => setTimeout(() => resolve('asyncTask result'), 0));
const promiseTask = new Promise<string>((resolve) =>
  setTimeout(() => resolve('promiseTask result'), 0)
);
...
suite('Queue', function () {
  ...
  suite('enqueue()', function () {
    ...
    test('should throw an error if first parameter is not a function or a promise', function () {
      // eslint-disable-next-line no-restricted-syntax, unicorn/no-null
      for (const value of [undefined, null, 'string', true, 1, [], {}, true]) {
        // @ts-expect-error: parameter of type '() => unknown'. Type 'undefined' is not assignable to type '() => unknown'.ts(2345)
    ...
    test('should return a promise that resolves with the result of the task', function () {
      ...
      const promiseTaskResultPromise = queue.enqueue(
        promiseTaskSpy as Promise<string>
      );

      const assertedsyncTaskResultPromise = syncTaskResultPromise.then((result) => {
        const syncTaskSpyResult = syncTaskSpy.returnValues[0];
        if (syncTaskSpyResult) assert.strictEqual(result, syncTaskSpyResult);
        else throw new Error('syncTaskSpyResult should exist');
        return result;
      });

      const assertedAsyncTaskResultPromise = asyncTaskResultPromise.then(
        (result) => {
      ...
      const assertedPromiseTaskResultPromise = promiseTaskResultPromise.then(
        (result) => {
